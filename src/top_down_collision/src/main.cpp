...
int main()
{
    ...
    bn::sprite_ptr chest_sprite = bn::sprite_items::chest.create_sprite(0, 0);
    ...
    bn::point chest_map_position(dog_map_position.x() + 2, dog_map_position.y());
    while (true)
    {
        ...
        {
            // Check if the dog is trying to move into the chest
            if (new_dog_map_position == chest_map_position)
            {
                // Calculate the position behind the chest based on the dog's movement direction
                bn::point direction = new_dog_map_position - dog_map_position;
                bn::point new_chest_map_position = chest_map_position + direction;

                // Check if the position behind the chest is valid (not a wall)
                bn::regular_bg_map_cell chest_next_cell = map_item.cell(new_chest_map_position);
                int chest_next_tile_index = bn::regular_bg_map_cell_info(chest_next_cell).tile_index();
                ...
                // if the position behind the chest is not valid, the dog cannot move into the chest, so we do nothing
            }
            else
            {
                dog_map_position = new_dog_map_position;
        ...
        bn::fixed chest_sprite_x = (chest_map_position.x() * 8) - (map_item.dimensions().width() * 4) + 4;
        bn::fixed chest_sprite_y = (chest_map_position.y() * 8) - (map_item.dimensions().height() * 4) + 4;
